...
export default function SolicitudesLista({ navigation }) {
  ...
  const onRefresh = useCallback(() => {
    setRefreshing(true);
    // onSnapshot es en tiempo real; refresco ligero para UX
  ...
  const handleCancel = async (id) => {
    Alert.alert('Cancelar solicitud', '¿Estás segura de cancelar esta solicitud?', [
      { text: 'No', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            // opción simple: eliminar doc; alternativa: update status: 'cancelled'
  ...
  const renderItem = ({ item }) => {
    ...
    return (
      <TouchableOpacity style={styles.card} onPress={() => router.replace('/cliente/solicituddetalle')}>
        ...
        <View style={styles.actions}>
          <TouchableOpacity style={styles.actionBtn} onPress={() => router.replace('/cliente/solicituddetalle')}>
